...
export default function ProfileTypes(props) {
  ...
  const updateRecord = (e, id) => {
    e.preventDefault();
    router.push(`/panel/profiletypes/update/${id}`);
  };
  ...
  const handleDelete = async (d) => {
    ...
    //POST form values
    const res = await fetch("/api/profiletypes/" + uuid, {
      method: "DELETE",
      headers: {
  ...
  return (
    <AdminContainer>
      ...
      <Row>
        <Link href="/panel/profiletypes/create" passHref={true}>
          <Button className="btn btn-default" color="primary">
            Crear Tipo de Perfil
          </Button>
        </Link>
      </Row>
      <Table hover>
...
export async function getServerSideProps(ctx) {
  ...
  let r = await fetch(baseurl +"/api/profiletypes");
  if (r.status === 500) {
    return {
      props: {
        data: [],
  ...
  if (r.status === 404) {
    return {
      props: {
        data: [],
        error: "HTTP 404: Not found /api/profiletypes",
      },
    };
  }
