...
export default function Languages(props) {
  ...
  const updateRecord = (e, id) => {
    e.preventDefault();
    router.push(`/panel/languages/update/${id}`);
  };
  ...
  const handleDelete = async (d) => {
    ...
    //POST form values
    const res = await fetch("/api/languages/" + uuid, {
      method: "DELETE",
      headers: {
  ...
  return (
    <AdminContainer>
      ...
      <Row>
        <Link href="/panel/languages/create" passHref={true}>
...
export async function getServerSideProps(ctx) {
  ...
  let r = await fetch(baseurl + "/api/languages");
  if (r.status === 500) {
    return {
      props: {
        data: [],
  ...
  if (r.status === 404) {
    return {
      props: {
        data: [],
        error: "HTTP 404: Not found /api/languages",
      },
    };
  }
