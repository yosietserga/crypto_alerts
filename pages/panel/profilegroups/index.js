...
export default function ProfileGroups(props) {
  ...
  const updateRecord = (e, id) => {
    e.preventDefault();
    router.push(`/panel/profilegroups/update/${id}`);
  };
  ...
  const handleDelete = async (d) => {
    ...
    //POST form values
    const res = await fetch("/api/profilegroups/" + uuid, {
      method: "DELETE",
      headers: {
  ...
  return (
    <AdminContainer>
      ...
      <Row>
        <Link href="/panel/profilegroups/create" passHref={true}>
          <Button className="btn btn-default" color="primary">
            Crear Grupo de Perfil
          </Button>
        </Link>
      </Row>
      <Table hover>
...
export async function getServerSideProps(ctx) {
  ...
  let r = await fetch(baseurl + "/api/profilegroups?include=profileType");
  if (r.status === 500) {
    return {
      props: {
        data: [],
  ...
  if (r.status === 404) {
    return {
      props: {
        data: [],
        error: "HTTP 404: Not found /api/profilegroups",
      },
    };
  }
