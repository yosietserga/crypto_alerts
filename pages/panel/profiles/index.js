...
export default function Profiles(props) {
  ...
  const updateRecord = (e, id) => {
    e.preventDefault();
    router.push(`/panel/profiles/update/${id}`);
  };
  ...
  return (
    <AdminContainer>
      ...
      <Row>
        <Link href="/panel/profiles/create" passHref={true}>
...
export async function getServerSideProps(ctx) {
  ...
  let r = await fetch(baseurl + "/api/profiles?include=person,profileType");
  if (r.status === 500) {
    return {
      props: {
        data: [],
  ...
  if (r.status === 404) {
    return {
      props: {
        data: [],
        error: "HTTP 404: Not found /api/profiles",
      },
    };
  }
