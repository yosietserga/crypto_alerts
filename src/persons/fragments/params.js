...
export function buildFiltersQueryString(f) {
  ...
  if (isset(f.id) && !empty(f.id)) {
    ...
    const dateFields = ["createdAt", "updatedAt", "dateOfBirthday"];
    ...
    const numbersAsArraysFields = ["storeId"];
    ...
    const stringsAsArraysFields = ["firstname", "lastname", "email", "phone", "gender", "genderOfBirthday", "username"];
